#!/usr/bin/env python3
"""
Check portfolio for loss-recovery / rising-while-red / spike alerts and email them.

Usage (from project root):
  ./venv/bin/python scripts/check_alerts.py           # check + email
  ./venv/bin/python scripts/check_alerts.py --dry-run # print only, no email
  ./venv/bin/python scripts/check_alerts.py --seed    # mark current losers without mailing
...
def main() -> int:
    ...
    if not args.dry_run and not EmailService.is_configured():
        print(
            ...
            "ALERT_EMAIL_TO ekle.\n"
            "Önce denemek için: ./venv/bin/python scripts/check_alerts.py --dry-run",
            file=sys.stderr,
        )
